...
final class CloudFirestoreClient {
  CloudFirestoreClient.initialize(File serviceAccount) {
    final saJson = serviceAccount.readAsStringSync();
    final jwt = JwtServiceAccountAuthenticator(saJson);
    _firestore = Firestore(jwt.projectId!, authenticator: jwt.authenticate);
  }
  ...
  Future<DbEntityRecord?> _getById(
    ...
    try {
      ...
      return DbEntityRecord(id: document.id, data: document.map);
    } on GrpcError catch (_) {
      if (attempt < _maxRetries) {
